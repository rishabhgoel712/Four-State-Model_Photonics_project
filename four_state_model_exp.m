...
i=3;    %index for array
n=10000000;%total number of iterations
t(1)=0;  %time array
next(1)=1;%stores the sequence of states with initial state as 1
on(1)=0;
[next(2), tinc(1),tr,on1]=state_exp(next(1), W1,W2,tau1,tau2,taua,tau); %next state is determined
on(2)=on1;
t(2)=t(1)+tinc(1);
while i<=n
    [next(i), tinc(i-1),tr,on1]=state_exp(next(i-1),W1,W2,tau1,tau2,taua,tau); %iteratively next state is determined 
    trap=trap+tr;
    on(i)=on1;
    t(i)=t(i-1)+tinc(i-1);
    i=i+1;
