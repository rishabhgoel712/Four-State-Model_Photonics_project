...
i=2;    %index for array
n=2000000;%total number of iterations
t(1)=0;  %time array
...
%t(2)=t(1)+tinc(1);
while i<=n
    [next(i), tinc(i-1),tr,on1,f,num]=state_n1(next(i-1),W1,W2,tau1,tau2,taua,tau); %iteratively next state is determined 
    trap=trap+tr;
    on(i)=on1;
    t(i)=t(i-1)+tinc(i-1);
...
line_style = {'-','--','-.','-'};
 ...
 %   set(h_legend,'fontsize',16, 'box', 'off');
    ...
    figname_png = ['Intensity and time for 4 state model 7.png'];
    figname = ['Intensity and time for 4 state model 7'];
    width = 30;
    height = 10;
